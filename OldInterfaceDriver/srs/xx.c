...
//                        rdata = SpiaRegs.SPIRXBUF;
    ...
    while(1)
    {
        ...
        if (Xint1Count != Xint1CountOld)
        {
            ...
            //для ADIS16488
            DeltaXSek = DeltaXSek + X_GyroInt32/910 + z;   //0.02/65536(ед. разряда в градусах), 0.02/65536*60*60 (ед. разряда в секундах)  /100 - частота   1/число = ед.разряда
            DeltaYSek = DeltaYSek + Y_GyroInt32/910 + z;
            DeltaZSek = DeltaZSek + Z_GyroInt32/910 + z;

            X_Gyro = ((int32)((((Uint32)BRFData->X_GYRO_OUT)<<16)+((Uint32)BRFData->X_GYRO_LOW)))/910 + z;
            Y_Gyro = ((int32)((((Uint32)BRFData->Y_GYRO_OUT)<<16)+((Uint32)BRFData->Y_GYRO_LOW)))/910 + z;
            Z_Gyro = ((int32)((((Uint32)BRFData->Z_GYRO_OUT)<<16)+((Uint32)BRFData->Z_GYRO_LOW)))/910 + z;


            zGyroSum += Z_Gyro;
            zGyroCount++;

            if(zGyroCount == 1000 - 1)
            {
                for (zGyroCount = 0; zGyroCount < 300; zGyroCount++)
                    zGyroAvg = zGyroSum / 1000;
                zGyroSum = 0;
                zGyroCount = 0;
            }
