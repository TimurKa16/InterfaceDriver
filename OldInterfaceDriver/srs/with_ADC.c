...
//#include "hw_types.h"
//#endif
#define adcArrayLength 500
unsigned int adcTimer = 0;
long int average = 0;
unsigned int i = 0;
unsigned int adcPressure = 0;

unsigned int adcArray[adcArrayLength];
...
Init_ADC()
{
    ...
    AdcdRegs.ADCSOC0CTL.bit.CHSEL     = 0;    // C0
...
//    #endif
    ...
    IER = 0x000f; // Enable group 3 interrupts
   ...
   Stop();
    ...
    while(1)
    {
        adcArray[adcTimer] = AdcbResultRegs.ADCRESULT5;

        if(adcTimer == adcArrayLength - 1)
        {
            for (i = 0; i < adcArrayLength; i++)
                average += adcArray[i];

            average = average / adcArrayLength;
            adcPressure = average;
            average = 0;
            adcTimer = 0;
        }
        ...
        if (Xint1Count != Xint1CountOld)
        {
            ...
            My_Otvet32.adcPressure_OUT_Hi = (adcPressure>>8) & 0xFF;
            My_Otvet32.adcPressure_OUT_Lo = adcPressure & 0xFF;
...
void SwitchKomand32_Tx(void)
{
    ...
    My_Com_32_Tx[26] = My_Otvet32.adcPressure_OUT_Hi;
    My_Com_32_Tx[27] = My_Otvet32.adcPressure_OUT_Lo;
